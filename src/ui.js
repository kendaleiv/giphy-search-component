export default class UI {
  constructor(instance, configuration, giphyApi) {
    if (!instance) {
      throw new Error('instance must be provided');
    }

    if (!configuration) {
      throw new Error('configuration must be provided.');
    }

    if (!giphyApi) {
      throw new Error('giphyApi must be provided.');
    }

    this.instance = instance;
    this.configuration = configuration;
    this.giphyApi = giphyApi;

    this.base64PoweredByApi = '';
    this.containerId = 'giphy-search-component-results-container';
    this.searchId = 'giphy-search-component-search';
  }

  start(initialSearch) {
    const html = `
      <div id="${this.containerId}"></div>
      <input type="text" id="${this.searchId}" />
      <img src="data:image/png;base64,${this.base64PoweredByApi}" />`;

    this.instance.style.display = 'block';
    this.instance.style.height = this.configuration.height;
    this.instance.style.width = this.configuration.width;

    this.instance.innerHTML = html;
    this.render([]);

    this.instance.querySelector(`#${this.searchId}`).addEventListener('change', async evt => {
      let results = [];

      if (evt.currentTarget.value) {
        results = await this.giphyApi.search(evt.currentTarget.value);
      }

      this.render(results);
    });
  }

  render(results) {
    const container = this.instance.querySelector(`#${this.containerId}`);

    if (results && results.data) {
      container.innerHTML = results.data.map(x => `<img src="${x.images.preview_gif.url}" />`);
    } else {
      container.innerHTML = '';
    }

    console.log(container.innerHTML);
  }
}
